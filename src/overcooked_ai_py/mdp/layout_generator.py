...
class Grid(object):
    ...
    def is_valid_feature_location(self, location):
        ...
        if any(
            [
                ...
                if (CODE_TO_TYPE[self.terrain_at_loc(loc)] == EMPTY or CODE_TO_TYPE[self.terrain_at_loc(loc)] == ICE)
            ]
        ):
            return True
        else:
    ...
    def location_is_empty(self, location):
        ...
        with open(f"{PATH}/ovmdp_debug_joana.txt", "w") as f:
            f.write(str("estou ao pe do gelo?") + "\n" + str(self.mtx[x][y] == TYPE_TO_CODE[ICE]))
            f.close()
